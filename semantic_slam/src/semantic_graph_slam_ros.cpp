...
void semantic_graph_slam_ros::open(ros::NodeHandle n) {
  ...
  ukf_odom_pose_sub_ = n.subscribe(
      "/odometry/filtered", 1, &semantic_graph_slam_ros::UKFVIOCallback, this);
...
void semantic_graph_slam_ros::UKFVIOCallback(const nav_msgs::Odometry::ConstPtr& odom_msg) {
  const ros::Time& stamp = odom_msg->header.stamp;
  Eigen::Isometry3d odom;
  Eigen::MatrixXf odom_cov;
