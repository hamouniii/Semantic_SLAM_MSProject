...
XMLFileReader::XMLFileReader(std::string xml_file_str_in) { // throw(cvg_XMLFileReader_exception) {
    ...
    std::ifstream xml_fstream(xml_file_str.c_str());
...
double XMLFileReader::readDoubleValue(const std::vector<std::string> &value_location) { // throw(cvg_XMLFileReader_exception) {
    ...
    double result;
    try {
        result = cvg_string_to_double(readingValue);
...
int XMLFileReader::readIntValue(const std::vector<std::string> &value_location) { // throw(cvg_XMLFileReader_exception) {
    ...
    int result;
    try {
        result = cvg_string_to_int(readingValue);
    } catch(std::invalid_argument &e) {
...
double XMLFileReader::readDoubleValue(const std::string &value_location)
{
    return readDoubleValue( cvg_convert_input_string_to_string_stdvector(value_location) );
}

int XMLFileReader::readIntValue(const std::string &value_location)
{
    return readIntValue( cvg_convert_input_string_to_string_stdvector(value_location) );
}

std::string XMLFileReader::readStringValue(const std::string &value_location, std::string xmlfilereader_caller_function)
{
    return readStringValue( cvg_convert_input_string_to_string_stdvector(value_location), xmlfilereader_caller_function );
}
...
std::string XMLFileReader::exception_string(std::string xmlfilereader_caller_function, const std::vector<std::string> &value_location, std::string aditional_exception_description) {
    ...
    for (std::vector<std::string>::const_iterator it = value_location.begin(); it != value_location.end(); ++it) {
